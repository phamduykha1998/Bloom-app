...
function entWk(weekOffset){
  ...
  return STATE.entries.filter(function(e){return ds_.indexOf(e.date)>=0;});
}
...
function entMo(monthOffset){
  ...
  return STATE.entries.filter(function(e){
    var d = parseD(e.date);
    return d.getFullYear()===r.y && d.getMonth()===r.m;
  });
}
...
function goPage(name, btn, isSidebar){
  ...
  var navIdx = {day:0,week:1,month:2,kanban:3,dashboard:4,events:5}[name];
  if(navIdx !== undefined){
...
function openModal(){
  modalState = {type:null, result:null, editId:null};
  document.getElementById('iName').value = '';
  document.getElementById('iNote').value = '';
  document.querySelectorAll('#mbg .to').forEach(function(b){b.classList.remove('ts','tm');});
...
function renderSuggW(){
  ...
  var arr = modalState.type==='s' ? sales : modalState.type==='m' ? mkt : [];
...
function saveAct(){
  ...
  if(!modalState.type){ toast('Chọn loại Sales hoặc Marketing'); return; }
...
function renderDay(){
  ...
  sgEl.innerHTML = ''+
    '<div class="sg">'+
      '<div class="sc sale">'+
        ...
        '<div class="sc-s">hoạt động sales</div>'+
      '</div>'+
      '<div class="sc mkt">'+
        '<div class="sc-head">'+
          '<div class="sc-l">'+icn('megaphone')+'Marketing</div>'+
        '</div>'+
        '<div class="sc-n pu">'+c.m+'</div>'+
        '<div class="sc-s">hoạt động marketing</div>'+
      '</div>'+
    '</div>';
  ...
  if(ents.length === 0){
    ...
    listEl.innerHTML = ents.map(function(e){
      var typeTag = e.type==='s' ?
        '<span class="tg ts">'+icn('heart')+'Sales</span>' :
        '<span class="tg tm">'+icn('megaphone')+'Mkt</span>';
      var resTag = e.result ?
        '<span class="tg '+(RES_CLS[e.result]||'tno')+'">'+RES_MAP[e.result]+'</span>' :
        '<span class="tg tno">Chưa rõ</span>';
      return '<div class="wi">'+
        '<div class="wd '+e.type+'"></div>'+
        '<div class="wb">'+
          '<div class="wn">'+escapeHtml(e.name)+'</div>'+
          '<div class="wts">'+typeTag+resTag+'</div>'+
          (e.note ? '<div class="wnote">'+escapeHtml(e.note)+'</div>' : '')+
        '</div>'+
        '<button class="wdel" onclick="delEntry(\''+e.id+'\')" title="Xoá">'+icn('trash')+'</button>'+
      '</div>';
    }).join('');
  }
...
function kbBuildCol(title, titleCls, headCls, cntCls, colKey, dateLabel, smEntries, todos){
  ...
  if(smEntries.length){
    html += '<div class="kb-from-sm">— Từ Sale &amp; Marketing —</div>';
    smEntries.forEach(function(e){
      var tt = e.type==='s' ?
        '<span class="tg ts">'+icn('heart')+'S</span>' :
        '<span class="tg tm">'+icn('megaphone')+'M</span>';
      var res = e.result ? '<span class="tg '+(RES_CLS[e.result]||'tno')+'">'+RES_MAP[e.result]+'</span>' : '';
      html += '<div class="kb-card '+(e.type==='s'?'sale-card':'mkt-card')+'">'+
        '<div class="kb-card-row"><div style="flex:1">'+
          '<div class="kb-card-name">'+escapeHtml(e.name)+'</div>'+
          '<div class="kb-card-tags">'+tt+res+'</div>'+
...
function renderKanban(){
  ...
  setTimeout(function(){
    ...
    if(cols.length >= 2 && window.innerWidth < 1024){
      cols[1].scrollIntoView({inline:'start', behavior:'smooth', block:'nearest'});
    }
  }, 80);
}
...
function exportCSV(){
  var rows = [['Ngày','Loại','Tên','Kết quả','Ghi chú']];
  STATE.entries.forEach(function(e){
    rows.push([
      e.date,
      e.type==='s'?'Sales':'Marketing',
      (e.name||'').replace(/"/g,'""'),
      RES_MAP[e.result]||'',
      (e.note||'').replace(/"/g,'""')
    ]);
